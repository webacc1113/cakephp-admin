...
class CalculatorHelper extends Helper {
	...
	function get_business_date_after_x_days($start_date, $business_days) {	
	
		//use \BusinessDays\Calculator;
		$holidays = [
			new \DateTime('1970-01-01'),
			new \DateTime('1970-12-25'),
			new \DateTime('1970-07-04'),			
			new \DateTime('1970-11-11')
		];
		...
		$this->setStartDate(new \DateTime($start_date));
		$this->setFreeWeekDays($freeWeekDays); // repeat every week
		$this->setHolidays($holidays);         // repeat every year		

		$this->addBusinessDays($business_days);             // add X working days

		$result = $this->getDate();       // \DateTime
