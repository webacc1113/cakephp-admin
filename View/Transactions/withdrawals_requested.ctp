...
<div class="box">
	<div class="box-header">
		...
		<ul class="box-toolbar">
			<li>
				<?php echo $this->Html->link('<i class="icon-remove-sign"></i> Clear filters', array('action' => 'withdrawals_requested'), array('escape' => false)); ?>
			</li>
		</ul>
	</div>
		...
		<?php if (isset($data) && !empty($data)): ?>
			<div class="box">
				<table cellpadding="0" cellspacing="0" class="table table-normal">
					...
					<tbody>
						<?php $paypal_total = $gift_total = $dwolla_total = $grand_total = $mvpay_total = 0; ?>
						<?php foreach ($data as $date => $row): ?>
							<tr>
								...
								<td>
									<?php $mvpay = (isset($row['mvpay'])) ? $row['mvpay'] : 0; ?>
									$<?php echo number_format($mvpay / 100, 2); ?>
								</td>
								<td>
									<?php $sub_total = $paypal + $gift + $dwolla + $mvpay; ?>
						...
						<tr>
							...
							<td>$<?php echo number_format($mvpay_total / 100, 2); ?></td>
